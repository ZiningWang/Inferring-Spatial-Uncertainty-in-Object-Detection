...
const double MAX_DEPTH[3] = {30.0, 50.0, 70.0};      // maximum depth
...
const double MAX_H[3] = {2.5, 2.5, 2.5};
const double MAX_L[3] = {6.5, 6.5, 6.5};
const double MIN_L[3] = {3.0, 3.0, 3.0};
...
vector<tDetection> loadDetections(string file_name, bool &compute_aos,
  ...
  while (!feof(fp)) {
    ...
    if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                   ...
                   &d.box.x2, &d.box.y2, &d.h, &d.w, &d.l, &d.t3, &d.t1, &d.t2,
                   &d.ry, &d.thresh)==16) {
...
vector<tGroundtruth> loadGroundtruth(string file_name,bool &success) {
  ...
  while (!feof(fp)) {
    ...
    char str[255];
    double temp;
    if (fscanf(fp, "%s %lf %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
                   str, &g.truncation, &g.occlusion, &g.box.alpha,
                   &g.box.x1,   &g.box.y1,     &g.box.x2,    &g.box.y2,
                   &g.h,      &g.w,        &g.l,       &g.t3,
                   &g.t1,      &g.t2,        &g.ry,   &temp )==16) {
      g.box.type = str;
...
void cleanData(CLASSES current_class, const vector<tGroundtruth> &gt, const vector<tDetection> &det, vector<int32_t> &ignored_gt, vector<tGroundtruth> &dc, vector<int32_t> &ignored_det, int32_t &n_gt, DIFFICULTY difficulty){
  ...
  for(int32_t i=0;i<gt.size(); i++){
    ...
    bool ignore = false;
    if(gt[i].t1<-40.0 || gt[i].t1>40.0 || gt[i].t3>MAX_DEPTH[difficulty] || gt[i].h>=MAX_H[difficulty] || gt[i].l>=MAX_L[difficulty] || gt[i].l<=MIN_L[difficulty]){
      ignore = true;
      n_ignored_gt++;
    }
    //WZN: for ignore debug
    //cout << "DEBUG: class: " << gt[i].box.type.c_str() << ", valid_class: " << valid_class << ", ignore: " << ignore << ", height: " << height << endl;
  ...
  for(int32_t i=0;i<det.size(); i++){
    ...
    if(det[i].t3>MAX_DEPTH[difficulty])
  ...
  //cout<<"num of gt: " << n_gt << ", # of ignored: " << n_ignored_gt << endl;
}
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  for (int32_t i=0; i<indices.size(); i++) {
    ...
    // file name
    char file_name[256];
    sprintf(file_name,"%08d.txt",indices.at(i));
    //cout << file_name << endl;
