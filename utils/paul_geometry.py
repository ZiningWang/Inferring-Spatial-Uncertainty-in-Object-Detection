...
#from helpers import pyxyz, cy_3d_iou, cy_2d_iou
'''
mod = SourceModule("""
...
def gpu_voxel(points, 
    ...
    cprint("gpu points : {}".format(len(points)))
...
# # --------------- method 1: Pyramid Grid search
# def figure_out_xyz(objects, coords, P, nbinx, nbinz, depth, image_size):
#     # x, y, z coordinate optimization
#     """
#     rangex: linear space of [min, max, n] in x axis
#     coords: N x [x, y, x, y]
#     """

#     def pyramid_search(rangex, rangez, depth, so_far=None):
#         print ('xsearch in', rangex, 'zsearch in', rangez)
#         if so_far is None: so_far = [0, 80, 0.]  # best_x, best_z, best_score
#         updated = False
#         for x in np.linspace(rangex[0], rangex[1], rangex[2]):
#             for y in np.linspace(1.5, 1.9, 3):
#                 for z in np.linspace(rangez[0], rangez[1], rangez[2]):
#                     obj.z = z
#                     obj.x = x
#                     obj.y = y
#                     box2d = get_coord_2d(obj, P, image_size)
#                     if box2d is None: continue
#                     score = bbox_iou(box2d, coord)
#                     if so_far[2] < score:
#                         so_far = (x, z, score)
#                         updated = True
#                         print ('search', depth, so_far[2])
#         if not updated or depth == 0:
#             return obj
#         else:
#             rangex = [so_far[0] - 2*(rangex[1]-rangex[0])/rangex[2],
#                       so_far[0] + 2*(rangex[1]-rangex[0])/rangex[2], rangex[2]]
#             rangez = [so_far[1] - 2*(rangez[1]-rangez[0])/rangez[2],
#                       so_far[1] + 2*(rangez[1]-rangez[0])/rangez[2], rangez[2]]
#             return pyramid_search(rangex, rangez, depth-1, so_far)

#     references = np.hstack([(coords[:, [0]] + coords[:, [2]])/2., coords[:, [3]]])
#     references = np.hstack([references, np.ones((len(references), 1))])
#     back_projections = references.dot(np.linalg.pinv(P).T)
#     back_projections *= (2. / back_projections[:,[1]])
#     print(back_projections)

#     back_xs = back_projections[:, 0]
#     back_zs = back_projections[:, 2]

#     for obj, coord, bx, bz in zip(objects, coords, back_xs, back_zs):
#         print('---------------------------------------')
#         obj = pyramid_search((bx-3, bx+3, nbinx), (bz + 0.5 * obj.length - 4, bz + 0.5 * obj.length + 4, nbinz), depth)

#     return objects

# # --------------- method 2: Optimization
# def figure_out_xyz(objects, coords, P, image_size):
#     # x, y, z coordinate optimization
#     """
#     rangex: linear space of [min, max, n] in x axis
#     coords: N x [x, y, x, y]
...
#     references = np.hstack([references, np.ones((len(references), 1))])
#     back_projections = references.dot(np.linalg.pinv(P).T)
#     back_projections *= (1.7 / back_projections[:,[1]])
#     print(back_projections)

#     back_xs = back_projections[:, 0]
#     back_zs = back_projections[:, 2]
#     back_ys = np.array([1.7] * len(back_xs))

#     def get_score(x0):
#         obj.x, obj.y, obj.z = x0
#         box2d = get_coord_2d(obj, P, image_size)
#         score = bbox_iou(box2d, coord)
#         return -score
...
#         y = 1.7
#         z = bz + obj.length * 0.5
#         res = minimize(get_score, [x, y, z], method='Nelder-Mead')
#         obj.x, obj.y, obj.z = res.x
#         print (get_score( (x,y,z) ))
#         print (get_score( res.x ))
#     return objects
